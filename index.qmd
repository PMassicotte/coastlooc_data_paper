...
```{r}
knitr::include_graphics(pdf2png(here("graphs", "17_boxplot_bathymetry_by_area.pdf")))
```
...
```{r}
knitr::include_graphics(pdf2png(here("graphs", "14_total_chla_vs_aphy443.pdf")))
```
...
knitr::include_graphics(pdf2png(
  here(
    "graphs",
    "15_absorption_partition_for_three_stations.pdf"
  )
))
```
...
- There are only three AC9 measurements in the Adriatic Sea. Is it normal?
...
- Difference between `pico_chl_a` and `pic`, `nano_chl_` and `nano`?
