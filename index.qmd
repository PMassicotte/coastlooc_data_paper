---
title: "Coastlooc data paper"
format: 
  html:
    toc: true
    self-contained: true
    fig-format: svg
    # fig-dpi: 300
editor_options: 
  chunk_output_type: console
bibliography: /home/filoche/Documents/library.bib
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(
  echo = FALSE,
  message = FALSE,
  warning = FALSE,
  fig.pos = "center",
  cache = TRUE
)

library(tidyverse)
library(here)
library(sf)
library(kableExtra)
library(arrow)

library(ggpmthemes)
extrafont::loadfonts()
# Set default ggplot2 font size and font family
# devtools::install_github("PMassicotte/ggpmthemes")
theme_set(theme_light_modified(
  base_family = "Poppins",
  base_size = 10
))

theme_update(
  panel.border = element_blank(),
  axis.ticks = element_blank(),
  strip.background = element_blank(),
  strip.text = element_text(face = "bold", size = 10, color = "#4c4a4a")
)

source(here("R", "zzz.R"))
```

## Data cleaning and pre-processing

This section shows the main steps that have been applied to pre-process the raw data.

### aCDOM spectra

- The CDOM spectra were modelled according to the information in @.

    - `acdom` spectra were re-fitted using the complete data (i.e. between 350-500 nm) because the data in `all_abs_transpose.txt` started at 380 nm.

- Average background values calculated between 683-687 nm and subtracted from each spectrum.

- Some files were in binary format, so I could not open them (ex.: `C2001000.YSA`).

- Some spectra start at 300 nm while others at 350 nm.

- Calculated the correlation between the measured and the fitted values.

    - Fits with R2 lower than 0.95 were removed from the data.

- Absorption spectra with any negative values below 500 nm were removed.

- Exported the complete spectra (350-700 nm): both the raw and the modelled data.

### Phytoplankton and non-algal absorption

- Absorption spectra with any negative values below 500 nm were removed.

### Irradiance

- There were negative values in the irradiance data (`Ed`, `Eu`, `Kd`, `Ku`). I have cleaned the data by setting these negative values to `NA`.

This graph shows the number of negative values for Ed by wavelength.

```{r histo_negative_ed}
irradiance <- read_csv(here("data", "raw", "tidied", "irradiance.csv")) |> 
  filter(ed_w_m2_um < 0)

irradiance |> 
  ggplot(aes(x = ed_w_m2_um)) +
  geom_histogram() +
  facet_wrap(~ glue::glue("{wavelength} nm"), scales = "free")
```

- Example of a spectral profile with negative values.

```{r lineplot_negative_ed}
read_csv("./data/raw/tidied/irradiance.csv") |> 
  filter(station == "C2008000") |> 
  ggplot(aes(x = wavelength, y = ed_w_m2_um)) +
  geom_line() +
  geom_point() +
  labs(
    title = "Station C2008000",
    x = "Wavelength (nm)",
    y = parse(text = "E[d](0^'-')~'['~W~m^{-2}~nm^{-1}~']'")
  )
```

- `Eu` is in fact `Eu0-` that was estimated using a two-exponential function model.

- `Ed` is in fact `Ed0-` calculated from `0.94 x ed0+`.

- There are differences in wavelengths among cruises. I have not found any information in the report concerning channel change across the missions.

```{r}
irradiance <- read_csv("data/clean/irradiance.csv")

irradiance |> 
  mutate(cruise = str_sub(station, 1, 2), .after = station) |> 
  distinct(cruise, wavelength) |> 
  group_nest(cruise) |> 
  unnest_wider(data) |> 
  pivot_wider(names_from = cruise, values_from = wavelength) |> 
  unnest(everything()) |> 
  knitr::kable() |> 
  kableExtra::kable_styling()

irradiance |> 
  mutate(cruise = str_sub(station, 1, 2), .after = station) |> 
  distinct(cruise, wavelength) |> 
  group_nest(cruise) |> 
  unnest_wider(data) |> 
  pivot_wider(names_from = cruise, values_from = wavelength) |> 
  unnest(everything()) |> 
  rowwise() |> 
  mutate(u = length(unique(c_across(everything())))) |> 
  filter(u != 1) |> 
  select(-u) |> 
  knitr::kable() |> 
  kableExtra::kable_styling()
```

### Reflectance

- Reflectance values outside the 0-1 range were set to `NA`.

### AC9

- Negative values in `a`, `c`, `bp`, `a_dissolved` and `c_dissolved` have been set to `NA`.

- `a(715)` was used as the baseline, that is why the values are always at 0 (see next graph).

```{r}
knitr::include_graphics(pdf2png(
  here("graphs", "07_histogram_ac9_negative_values_removed.pdf")
))
```

### Other stuff

- Extracted extra variables (`DOC`, `AQY`) from Massimo 2000.

## Data sampling

Just some graphs to visualize the data. *Note that the same colour palette will be used to represent the areas in all graphics.*

### Temporal sampling

This graph shows when the sampling was performed in the different areas. For instance, we can see that a large fraction of the measurements were made in September of 1998.

```{r}
source(here("R", "zzz.R"))

df <- read_csv(here("data", "clean", "stations.csv"))

df_viz <- df |> 
  mutate(month = clock::date_group(date, "month")) |> 
  count(area, month)

df_viz |> 
  ggplot(aes(x = n, y = factor(month), fill = area)) +
  geom_col() +
  scale_fill_manual(
    breaks = area_breaks,
    values = area_colors
  ) +
  labs(
    x = "Number of observations",
    y = NULL,
    title = "Number of measurements by month"
  ) +
  theme(
    legend.position = "top",
    legend.title = element_blank()
  )
```

### Bathymetry

I have extracted the bathymetry at each sampling locations. This boxplot provide a general picture of the bathymetry per area. Data from https://download.gebco.net/.

```{r}
knitr::include_graphics(pdf2png(here("graphs", "18_boxplot_bathymetry_by_area.pdf")))
```

## Available variables

This graph shows an overview of the available variables (excluding radiometric measurements).

```{r}
knitr::include_graphics(pdf2png(
  here("graphs", "04_number_observation_nutrient_by_area.pdf")
))
```

## Absorption measurements

Overview of the averaged absorption spectra for each area.

```{r}
knitr::include_graphics(pdf2png(here(
  "graphs", "12_average_absorption_spectra_by_area.pdf"
)))
```

Comparing `acdom443` for the different areas shows that there is a clear open to coastal gradient.

```{r}
knitr::include_graphics(pdf2png(here(
  "graphs", "14_boxplot_acdom443_by_area.pdf"
)))
```

We can see that the `DOC` follows the same pattern as `acdom443`.

```{r}
knitr::include_graphics(pdf2png(here("graphs", "13_boxplot_doc_by_area.pdf")))
```

We can also use scatter plots to further explore the relationships among variables.

```{r}
knitr::include_graphics(pdf2png(here("graphs", "14_aphy_vs_anap.pdf")))
```

### aphy

```{r}
knitr::include_graphics(pdf2png(here("graphs", "09_aphy_spectral_profiles_by_area.pdf")))
```

We could also assess the goodness of the relationships between total chlorophyll-a and phytoplankton absorption for each region.

```{r}
knitr::include_graphics(pdf2png(here("graphs", "15_total_chla_vs_aphy443.pdf")))
```

### anap

```{r}
knitr::include_graphics(pdf2png(here(
  "graphs", "09_anap_spectral_profiles_by_area.pdf"
)))
```

### ap

```{r}
knitr::include_graphics(pdf2png(here(
  "graphs", "09_ap_spectral_profiles_by_area.pdf"
)))
```

### acdom

```{r}
knitr::include_graphics(pdf2png(here(
  "graphs", "09_acdom_spectral_profiles_by_area.pdf"
)))
```

### Absorption partition

In this section I am using the same three stations as in @Oubelkheir2007 to explore the additive contributions of each type of absorption.

- For station `C6024000`, `a_p` is lower than `a_cdom` around 400 nm and 550 nm. Should we use this to filter out problematic spectra?

- There are some obvious problems with `a_cdom` measurements. See `C6024000` where there is a bump in absorption around 550 nm.

```{r}
knitr::include_graphics(pdf2png(
  here(
    "graphs",
    "16_absorption_partition_for_three_stations.pdf"
  )
))
```

### Spectral slopes

This graph compares the spectral slopes of both `CDOM` and `NAP` absorption spectra.

```{r, fig.align='center'}
s_cdom <- read_csv(here("data", "clean", "s_cdom.csv"))
s_nap <- read_csv(here("data", "clean", "s_nap.csv"))
stations <- read_csv(here("data", "clean", "stations.csv"))

df <- stations |> 
  inner_join(s_cdom, by = "station") |> 
  inner_join(s_nap, by = "station")

df |> 
  select(station, area, s_cdom_nm1, s_nap_nm1) |> 
  pivot_longer(starts_with("s_")) |> 
  ggplot(aes(x = name, y = value, fill = area)) +
  geom_boxplot(size = 0.1, outlier.size = 0.25) +
  facet_wrap(~area) +
  scale_fill_manual(
    breaks = area_breaks,
    values = area_colors
  ) +
  scale_x_discrete(
    breaks = c("s_cdom", "s_nap"),
    labels = c(parse(text = "S[CDOM]"), parse(text = "S[NAP]"))
  ) +
  labs(
    x = NULL,
    y = quote("Spectral slope" ~ (nm^{
      -1
    }))
  ) +
  theme(
    legend.position = "none"
  )
```

## Irradiance

### Ed

```{r}
knitr::include_graphics(pdf2png(here(
  "graphs", "05_ed_spectral_profiles_by_area.pdf"
)))
```

### Eu

```{r}
knitr::include_graphics(pdf2png(here("graphs", "05_eu_spectral_profiles_by_area.pdf")))
```

### Kd

```{r}
knitr::include_graphics(pdf2png(here("graphs", "05_kd_spectral_profiles_by_area.pdf")))
```

### Ku

```{r}
knitr::include_graphics(pdf2png(here("graphs", "05_ku_spectral_profiles_by_area.pdf")))
```

## Reflectance

```{r}
knitr::include_graphics(pdf2png(here("graphs", "06_reflectance_profiles_by_area.pdf")))
```

## AC9

### Absorption

```{r}
knitr::include_graphics(pdf2png(here("graphs", "07_ac9_a_spectral_profiles_by_area.pdf")))
```

### Beam attenuation

```{r}
knitr::include_graphics(pdf2png(here("graphs", "07_ac9_c_spectral_profiles_by_area.pdf")))
```

### Scattering

```{r}
knitr::include_graphics(pdf2png(here("graphs", "07_ac9_bp_spectral_profiles_by_area.pdf")))
```

## Journal candidates

- [Scientific data](https://www.nature.com/sdata/) (I.F: 5.541)
- [Earth System Science Data (ESSD)](https://earth-system-science-data.net/) (I.F: 9.197)
- [PLOS One](https://journals.plos.org/plosone/) (I.F: 2.740)

## Figures for the paper

This section shows the figures that I think should be included in the data paper.

### Figure 1

Figure 1: Map of the sampling stations.

```{r fig01}
knitr::include_graphics(pdf2png(here("graphs", "fig01.pdf")))
```

### Figure 2

Figure 2: (**A**) Overview of the temporal sampling for the seven areas. The numbers in the circles indicate the number of visited stations each month. (**B**) Boxplot showing the bathymetry at the sampling locations by area.

```{r fig02}
knitr::include_graphics(pdf2png(here("graphs", "fig02.pdf")))
```

### Figure 3

Figure 3: (**A**) Total chlorophyll-a and (**B**) particulate organic carbon across the sampled areas.

- Is it normal there is no data for `Med. Sea (Case 1)`?

```{r fig03}
knitr::include_graphics(pdf2png(here("graphs", "fig03.pdf")))
```

### Figure 4

Figure 4: (**A**) Average total particulate ($a_\text{p}$), (**B**) non-algal ($a_\text{NAP}$), (**C**) phytoplankton ($a_{\phi}$) and (**D**) chromophoric dissolved organic matter ($a_\text{CDOM}$) absorption spectra in each area. (**E**) $a_\text{CDOM}(350)$ along the westernmost transect in the North Sea (see Fig. 1B).

- Is it normal there is no data for `Med. Sea (Case 1)`?

```{r fig04}
knitr::include_graphics(pdf2png(here("graphs", "fig04.pdf")))
```

### Figure 5

Figure 5: (**A**) Particulate scattering coefficient at 440 nm ($b_{b}(440)$) and (**B**) attenuation coefficient for downward irradiance at 443 nm ($K_{d}(443)$) across the sampled areas.

```{r fig05}
knitr::include_graphics(pdf2png(here("graphs", "fig05.pdf")))
```

### Figure 6

Figure 6: Spectra of downward irradiance measured at the surface across the sampled areas. The tick lines represent the average spectra.

- Dot not forget to find out why there are duplicated spectra in Med. Sea.

```{r fig06}
knitr::include_graphics(pdf2png(here("graphs", "fig06.pdf")))
```

### Figure 7

Scatterplots showing relationships among different selected variables. (**A**) Particulate organic carbon (POC) and (**B**) phytoplankton absorption at 443 nm ($a_{\phi}(443)$) against total chlorophyll-a. (**C**) Downward irradiance at 443 nm ($E_{d}(443)$) and (**D**) particulate scattering at 440 nm ($b_{b}(440)$ against particulate organic carbon. The red lines show the linear relationships between the variables. The shaded gray areas represent the 95% confidence intervals around the fitted models.

```{r fig07}
knitr::include_graphics(pdf2png(here("graphs", "fig07.pdf")))
```

## Done

- [x] Calculate `s_nap` and `s_cdom`. See the method in @Babin2003 where he removes some wavelengths to calculate `s_nap`.

- [x] Removed dissolved `a` and `c` from the AC9 data because there were problems with the filtering procedure during the sampling. These correspond to the `ad` and `cd` variables in `SurfaceData5(C4corr).txt`.

- [x] Extract bathymetry at each station.

- [x] Zoom on geographic areas in Fig. 1 such as figure 13 in the final report.

- [x] Recode Ed wavelengths from the SPMR *vertical* profiles as:

    - 412 -\> 411
    - 510 -\> 509
    - 589 -\> 590
    - 666 -\> 665
    - 780 -\> 779

- [x] Some geographical positions are located on land. I removed these observations from the dataset. A few stations had no coordinate at all, they were also removed.

```{r}
stations <- read_csv("data/raw/tidied/stations.csv")

stations_sf <- stations |>
  filter(str_detect(area, "Adriatic")) |>
  drop_na(longitude, latitude) |>
  st_as_sf(coords = c("longitude", "latitude"), crs = 4326)

crsuggest::suggest_crs(stations_sf)

wm <- rnaturalearth::ne_countries(scale = "large", returnclass = "sf") |>
  st_make_valid() |>
  st_crop(stations_sf |> st_buffer(1e5))

ggplot() +
  geom_sf(data = wm, size = 0.1) +
  geom_sf(data = stations_sf) +
  coord_sf(crs = 7792) +
  labs(
    title = "Geographical positions in the Adriatic"
  )
```

## TODO

- Use `piggyback` <https://docs.ropensci.org/piggyback/>

- Release the final report PDF with a DOI and cite it in the main text.
  - Who should be included in the release of this document?

- No absorption for `Med. Sea (Case 1)`. Is it normal?

```{r}
stations <- read_csv("data/clean/stations.csv") |>
  select(station, area)

read_csv("data/clean/absorption.csv") |>
  select(station, wavelength, a_p_m1, a_phy_m1, a_nap_m1, a_cdom_measured_m1) |>
  inner_join(stations, by = "station") |>
  pivot_longer(-c(station, wavelength, area)) |>
  drop_na() |>
  group_by(area, name) |>
  summarise(count_distinct = n_distinct(station)) |>
  ggplot(aes(x = count_distinct, y = area, fill = name)) +
  geom_col(position = "dodge") +
  labs(
    title = "No absorption measurements\nin Med. Sea (Case 1)"
  ) +
  scale_fill_viridis_d(option = "C")
```

- There are a lot of nutrient parameters that have values of zero. Are they true zero or they indicate missing values?

```{r}

nutrient <- read_csv("./data/clean/surface.csv")

nutrient |>
  pivot_longer(-c(station)) |>
  drop_na() |>
  filter(value == 0) |>
  group_by(name) |>
  tally() |>
  mutate(name = fct_reorder(name, n)) |>
  ggplot(aes(x = n, y = name)) +
  geom_col() +
  labs(
    title = "Number of observations with\na value of 0"
  )
```

- There are wavelength gaps in the `AC9`, `irradiance` and `reflectance` data. Is that normal?

- Add units to each variable. For example `depth` should becomes `depth_m`.

- There are duplicated Ed spectra in the data.

```{r}
stations <- read_csv(here("data/clean/stations.csv"))
ed <- read_csv(here("data/clean/irradiance.csv"))

df <- ed |> 
  inner_join(stations) |> 
  filter(str_detect(area, "Case"))

df |> 
  ggplot(aes(x = wavelength, y = ed_w_m2_um, group = station)) +
  geom_line() +
  facet_wrap(~area)
```

These are the duplicated Ed stations.

```{r}
df |> 
  select(station, date, area, wavelength, ed_w_m2_um) |> 
  pivot_wider(names_from = wavelength, values_from = ed_w_m2_um) |> 
  janitor::get_dupes(matches("\\d{3}")) |>
  rmarkdown::paged_table()
```

This is the same for Eu.

```{r}
df |> 
  select(station, date, area, wavelength, eu_w_m2_um) |> 
  pivot_wider(names_from = wavelength, values_from = eu_w_m2_um) |> 
  janitor::get_dupes(matches("\\d{3}")) |> 
  rmarkdown::paged_table()
```

How to remove duplicates:

```{r}
#| echo: true
duplicated <- df |> 
  select(station, date, area, wavelength, ed_w_m2_um) |> 
  pivot_wider(names_from = wavelength, values_from = ed_w_m2_um) |> 
  janitor::get_dupes(matches("\\d{3}")) 

duplicated

df |>
  anti_join(duplicated)

```



- There are only two AC9 measurements in the Adriatic Sea. Is it normal?

```{r}
ac9 <- read_csv("data/clean/ac9.csv")
stations <- read_csv("data/clean/stations.csv")

df <- inner_join(stations, ac9, by = "station", multiple = "all")

df |>
  group_by(area) |>
  summarise(count_distinct = n_distinct(station,)) |>
  mutate(area = fct_reorder(area, count_distinct)) |>
  ggplot(aes(x = count_distinct, y = area)) +
  geom_col() +
  labs(
    title = "Number of AC9 profiles per area"
  )

```
- What is the difference between `chl_a`and `total_chl_a` ?

```{r}
surface <- read_csv("./data/clean/surface.csv")

surface |>
  ggplot(aes(x = total_chl_a, y = chl_a)) +
  geom_point() +
  geom_abline(lty = 2, color = "red")

surface |>
  ggplot(aes(x = total_chl_a, y = total_chloroa)) +
  geom_point() +
  geom_abline(lty = 2, color = "red")
```

- Difference between `pico_chl_a` and `pico`, `nano_chl_a` and `nano`?

```{r}
surface |> 
  ggplot(aes(x = nano, y = nano_chl_a)) +
  geom_point() +
  scale_x_log10() +
  scale_y_log10() +
  geom_abline(color = "red")
```

- Make sure that all variables (columns) in the CSV files have units specified in their names (ex.: `depth_m`, `a_cdom_m1`, ...).

- Make sure that SPMR units are OK. um vs nm. Change accordingly in the SPMR code file (`zzz_radiometer.R`).

- Units (guessed from he final report)

  - SPM: g m-3

  - What is the difference between total_chl_a and chl_a.

  - chla: mg m-3

  - total_pheo: mg m-3

  - All the \*xanthin: mg m-3

  - nano/pico: mg m-3 (page 77)

- According to the final report are are the wavelengths (see page 32)
  - AC9: 412, 440, 488, 510, 555, 630, 650 or 532, 665, 715
  - BB4: 415, 440, 510, 675 (**I do not have BB4 data at the moment**)
  - SPMR: 412, 443, 456, 490, 510, 532, 560, 620, 665, 683, 705, 780, 865

```{r}
stations <- read_csv(here("data/clean/stations.csv")) |>
  select(station, area)

ac9 <- read_csv(here("data/clean/ac9.csv"))

ac9 |>
  left_join(stations, by = "station")  |>
  select(station, area, wavelength, ends_with("m1")) |>
  pivot_longer(ends_with("m1")) |>
  drop_na(value, area) |>
  count(area, name, wavelength) |>
  ggplot(aes(x = n, y = factor(wavelength), fill = area)) +
  geom_col(position = "dodge") +
  scale_fill_manual(
    breaks = area_breaks,
    values = area_colors
  ) +
  labs(
    title = "Number of AC9 observations\nby area and wavelength"
  ) +
  facet_wrap(~name)
```

- AC9 stations that have no metadata (i.e. no entries in the stations.csv file).

```{r}
ac9 |>
  anti_join(stations, by = "station") |>
  distinct(station)
```

```{r}
#| eval: false
open_dataset(here("data/clean/spmr/")) |>
  select(wavelength) |>
  distinct() |>
  arrange(wavelength) |>
  collect()
```

- SPMR vertical profiles have been reprocessed. This means that the *original* surface data is not the same as the newly processed vertical profiles (i.e. extrapolation to the surface). How to deal with that?

- Some mismatch across wavelengths in the SPMR data.
  - Grey cells indicate missing observations.
  - There are +- 1 nm difference between upward and downward wavelengths.

```{r}
#| fig-height: 14
#| fig-width: 8
metadata <- read_csv(here("data", "clean", "stations.csv")) |>
  select(-depth_m)

df <- open_dataset(here("data", "clean", "spmr", "parquet")) |>
  left_join(metadata, by = "station") |>
  collect()

df |>
  select(area, station, wavelength, k_par_m1:kd_m1) |>
  pivot_longer(-c(station, wavelength, area)) |>
  drop_na() |>
  distinct(name, area, wavelength) |>
  mutate(available = name) |>
  complete(wavelength, area, name, fill = list(available = NA)) |>
  ggplot(aes(x = name, y = factor(wavelength), fill = available)) +
  geom_tile(color = "white", size = 0.5) +
  labs(
    x = NULL,
    y = "Wavelength (nm)"
  ) +
  facet_wrap(~area, ncol = 2) +
  theme(
    legend.position = "none"
  )
```

- Add units to all variables in the SPMR data

## References
