...
Figure 4: (**A**) Average total particulate ($a_\text{p}$), (**B**) non-algal ($a_\text{NAP}$), (**C**) phytoplankton ($a_{\phi}$) and (**D**) chromophoric dissolved organic matter ($a_\text{CDOM}$) absorption spectra in each area.
...
Figure 5: (**A**) Particulate scattering coefficient at 440 nm ($b_{b}(440)$) measured by the AC9 and (**B**) phytoplankton specific absorption at 443 nm ($a_{\phi}^*(443)$) across the sampled areas.
...
Figure 6: Distribution of (**A**) downward irradiance at 443 nm ($E_{d}(443)$) and (**B**) attenuation coefficient for downward irradiance at 443 nm ($K_{d}(443)$) across the sampled areas.
