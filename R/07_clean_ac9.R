...
df_viz <- ac9 |>
  select(-remote_sensed_vertical_layer_m) |>
...
ggsave(
  here("graphs", "07_histogram_raw_ac9.pdf"),
  device = cairo_pdf,
  width = 24,
  height = 10
)
...
p <- ac9_clean |>
  pivot_longer(-c(station, wavelength, remote_sensed_vertical_layer_m)) |>
  ggplot(aes(x = value)) +
  geom_histogram() +
  facet_wrap(~ glue("{name} ({wavelength} nm)"), scales = "free", ncol = 10) +
  labs(
...
ggsave(
  here("graphs", "07_histogram_ac9_negative_values_removed.pdf"),
  device = cairo_pdf,
  width = 24,
  height = 10
)
...
save_fun(p_a, here("graphs", "07_ac9_a_spectral_profiles_by_area.pdf"))
save_fun(p_c, here("graphs", "07_ac9_c_spectral_profiles_by_area.pdf"))
save_fun(p_bp, here("graphs", "07_ac9_bp_spectral_profiles_by_area.pdf"))
