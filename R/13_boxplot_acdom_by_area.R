...
p <- acdom |>
  ...
  drop_na(a_cdom_modeled_m1) |>
  # mutate(area = str_wrap(area, 12)) |>
  mutate(area = fct_reorder(area, a_cdom_modeled_m1, .fun = mean)) |>
  ggplot(aes(x = area, y = a_cdom_modeled_m1, fill = area)) +
  geom_boxplot(size = 0.25, outlier.size = 1) +
  labs(
    x = NULL,
    y = quote(a[CDOM](443) ~ "[" ~ m^{
      -1
    } ~ "]")
  ) +
  scale_fill_manual(
...
aphy |>
  filter(wavelength == 443) |>
  ggplot(aes(x = a_phy_m1, y = a_nap_m1, color = area)) +
  geom_point(size = 0.5) +
